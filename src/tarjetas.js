...
function crearTarjetas(listaProductos = carritoJson) {
  ...
  listaProductos.forEach((producto) => {
    ...
    const addToCart = document.createElement("button"); // botón de añadir al carrito
    const detalles = document.createElement("div"); // contenedor de los detalles del producto, tipo, nombre y precio

    // --- INICIO DE LÓGICA DE IMÁGENES RESPONSIVE ---
    ...
    sourceDesktop.media = "(min-width: 1024px)"; // lg:
    sourceDesktop.srcset = producto.image.desktop;

    sourceTablet.media = "(min-width: 768px)"; // md:
    sourceTablet.srcset = producto.image.tablet;

    // Imagen base / móvil
    foto.src = producto.image.mobile;
    foto.alt = `fotografia de ${producto.name}`;

    // Construimos el elemento picture
    picture.appendChild(sourceDesktop);
    picture.appendChild(sourceTablet);
    picture.appendChild(foto);

    // estilos en tailwind (Los estilos de la imagen se quedan en la etiqueta 'img')
    ...
    addToCart.className =
      "flex gap-2 absolute left-25 bottom-20 border rounded-full bg-white py-2 px-6";

    foto.className = " object-fill w-full"; // Añadido 'w-full' para que herede correctamente el ancho del contenedor picture
    tarjeta.className =
      "relative cursor-pointer max-w-90 md:max-w-65 bg-[#FCF9F7] rounded-md overflow-hidden gap-10 mb-5 shadow";
    ...
    // IMPORTANTE: Ahora agregamos el 'picture' en lugar de 'foto'
    tarjeta.appendChild(picture);
...
function aplicarFiltros() {
  ...
  if (rSeleccionada !== "" && rSeleccionada !== "default") {
    postresFiltrados = postresFiltrados.filter(
      (producto) => producto.price.toFixed(2) === rSeleccionada,
    );
  }

  if (busqueda !== "") {
    postresFiltrados = postresFiltrados.filter((producto) =>
...
function listaPrecios() {
  const preciosUnicos = [
    ...new Set(carritoJson.map((producto) => producto.price.toFixed(2))),
  ];
